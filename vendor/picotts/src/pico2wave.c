...
/* NAVIUS_PICO_LANG env var overrides the compiled-in lang dir at runtime */
const char * PICO_LINGWARE_PATH             = NULL;
...
int main(int argc, const char *argv[]) {
    /* Resolve lang dir: env var overrides compiled-in default */
    const char * env_lang = getenv("NAVIUS_PICO_LANG");
    if (env_lang) {
        static char lang_buf[512];
        snprintf(lang_buf, sizeof(lang_buf), "%s/", env_lang);
        PICO_LINGWARE_PATH = lang_buf;
    } else {
#ifdef picolangdir
        PICO_LINGWARE_PATH = picolangdir "/";
#else
        PICO_LINGWARE_PATH = "./lang/";
#endif
    }
